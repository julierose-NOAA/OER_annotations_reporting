...
#-------------------------------------------------------------------------------
#import
ROV_import_df <- ROV_import(paste0(ROV_filepath,dive_name,"_ROVtrack.csv"))
...
outlier_threshold <- max(ROV_SMA_df_outliers$distance) #use max b/c values are negative
