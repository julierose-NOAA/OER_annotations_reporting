...
dive_count <- nrow(summary_statistics)
science_dive_count <- dive_count - UCH_dives
...
This report is an expedition-level summary of expert annotations of benthic deep-sea communities and substrate for groups of ROV dives within the NOAA Ocean Exploration [ASPIRE](https://oceanexplorer.noaa.gov/explorations/aspire/welcome.html) campaign (2017-2022). These data have been QAQC\'d by the France Lab at the University of Louisiana Lafayette, following protocols described in the Annotating NOAA Ocean Exploration Video [manual](https://oceanexplorer.noaa.gov/okeanos/collaboration-tools/science_annotations/media/NOAA_Ocean_Exploration_Video_Annotations.pdf). These summarized annotations only represent the benthic portion of the dive, defined using the on- and off-bottom times provided in the [Dive Summary .txt files](https://www.ncei.noaa.gov/waf/okeanos-rov-cruises/) available for each dive. These summarized annotations do not include any midwater organisms that may have been observed on ROV descent/ascent or within midwater transects.

This report provides data visualizations in the form of tables and graphs of: 1. Dive summary metrics (ROV time on bottom, ROV mean depth); 2. Observational metrics by dive (total numbers of benthic biological and geoform/substrate annotations, percentage of annotations identified to each taxonomic level, phylum-level community composition, and numbers of annotations of taxonomic groups of interest, including Cnidaria, Porifera, Echinodermata, and Vertebrata); and 3. Quality control metrics (percentage of annotations flagged for review, number of unidentified biological annotations).
...
`r expedition_name` consisted of `r dive_count` total dives, of which `r science_dive_count` were science-focused and `r UCH_dives` explored underwater cultural heritage sites.
...
Add figure here for mean ROV depth while on bottom

###### [Figure 2]{custom-style="Strong"}. Mean ROV depth in meters.

The shallowest dive during `r expedition_name` had a mean depth of \[add link\], and the deepest dive had a mean depth of \[add link\]. Overall, dives within `r expedition_name` averaged \[add link\] meters in depth.
...
ggplot(summary_statistics, aes(x = dive_number, y = percent_flagged)) +
  ...
  ylim(0,100) +
  scale_x_continuous(n.breaks = nrow(summary_statistics)) +
  geom_text(aes(label = percent_flagged_rounded), vjust = 0, family = "Roboto", color = "#003087") + 
  OER_theme
```
