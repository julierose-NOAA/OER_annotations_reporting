...
ggplot(summary_statistics, aes(x = dive_number, y = Chordata)) +
  geom_col(fill = "#34bddf") +
  labs(x = "Dive Number", y = "Number of Annotations") +
  scale_x_continuous(n.breaks = nrow(summary_statistics)) +
  OER_theme

chordata_min <- min(summary_statistics$Chordata)
chordata_max <- max(summary_statistics$Chordata)
chordata_median <- median(summary_statistics$Chordata)
```
