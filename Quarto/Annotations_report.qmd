...
-   Observational metrics by dive (total numbers of benthic biological and geoform/substrate annotations; percentage of annotations identified to each taxonomic level; phylum-level community composition; numbers of annotations of taxonomic groups of interest, i.e., Cnidaria, Porifera, Echinodermata, and Vertebrata), and

-   Quality control metrics (percentage of annotations flagged for review; number of unidentified biological annotations).
...
###### [Figure 5]{custom-style="Strong"}. Phylum-level observations and community composition

Phyla observed during `r expedition_name` varied by dive and are illustrated here using color intensity, which indicates the relative contribution of each phylum to the total benthic biological annotations for each dive. The most common phyla observed during EX2104 can be identified by examining the variation in color intensity across rows. The community composition observed during EX2104 can be compared across dives by examining the variation in color intensity across columns.
...
Cnidaria is a phylum of particular interest as it contains a variety of habitat-building invertebrates commonly observed in the deep sea, including corals and sea anemones. The total number of cnidarians observed during each dive of `r expedition_name` ranged from `r cnidaria_min` to `r cnidaria_max` with a median of `r cnidaria_median` cnidarian observations across all dives.
...
The phylum Porifera includes sponges. The total number of porifera observed during each dive of `r expedition_name` ranged from `r porifera_min` to `r porifera_max` with a median of `r porifera_median` porifera observations across all dives.
...
The phylum Echinodermata includes a variety of invertebrates common to the deep sea, including sea stars (Class: Asteroidea), brittle stars (Class: Ophiuroidea), crinoids (Class: Crinoidea), sea urchins (Class: Echinoidea), and sea cucumbers (Class: Holothuroidea). The total number of echinoderms observed during each dive of `r expedition_name` ranged from `r echinodermata_min` to `r echinodermata_max` with a median of `r echinodermata_median` echinoderm observations across all dives.
...
The subphylum Vertebrata contains all vertebrates within the phylum Chordata. Commonly observed vertebrates include fishes. The total number of vertebrates observed during each dive of `r expedition_name` ranged from `r vertebrata_min` to `r vertebrata_max` with a median of `r vertebrata_median` vertebrate observations across all dives.
